...
export async function uploadEventImage(formData: FormData): Promise<UploadResponse> {
  ...
  if (!eventId) {
    return { error: "ID del evento es requerido" };
  }
  ...
  try {
    ...
    console.error("[UPLOAD_EVENT_IMAGE_ERROR]", error);
