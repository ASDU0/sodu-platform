...
export async function uploadWorkshopImage(formData: FormData): Promise<UploadResponse> {
  ...
  if (!workshopId) {
    return { error: "ID del taller es requerido" };
  }
  ...
  try {
    ...
    console.error("[UPLOAD_WORKSHOP_IMAGE_ERROR]", error);
